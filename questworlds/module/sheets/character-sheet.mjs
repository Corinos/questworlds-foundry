...
export class QWCharacterSheet extends HandlebarsApplicationMixin(ActorSheetV2) {
  ...
  _getTabs() {
    return this._prepareTabs("primary");
  }
...
console.log("QW | _onRender fired - questworlds/questworlds/module/sheets/character-sheet.mjs");
  console.log("QW | tabGroups:", this.tabGroups);
  console.log("QW | static TABS:", QWCharacterSheet.TABS);
  
  const navItems = html.querySelectorAll(".sheet-tabs .item[data-tab]");
  console.log("QW | nav tab elements found:", navItems.length);
  navItems.forEach(el => {
    console.log(`QW |   tab="${el.dataset.tab}" classes="${el.className}"`);
    el.addEventListener("click", () => console.log(`QW | CLICKED tab: ${el.dataset.tab}`));
  });

  const tabPanels = html.querySelectorAll(".sheet-body .tab");
  console.log("QW | tab panels found:", tabPanels.length);
  tabPanels.forEach(el => {
    console.log(`QW |   panel tab="${el.dataset.tab}" active=${el.classList.contains("active")}`);
  });
    // Inline editing
